...
public class RaycastRenderer extends Renderer implements TFChangeListener {
    ...
    void slicer(double[] viewMatrix , int a) {
        ...
        for (int j = 0; j < image.getHeight(); j++) {
            for (int i = 0; i < image.getWidth(); i++) {
                for (int t = 0; t <= nSlices ;t++ ){
                    double zValue = t * (volume.getDimZ()/2) / (double) nSlices ; //double nslices cause math
                    ...
                    pixelCoord[0] = uVec[0] * (i - imageCenter) + vVec[0] * (j - imageCenter)
                     ...
                     pixelCoord[1] = uVec[1] * (i - imageCenter) + vVec[1] * (j - imageCenter)
                        + volumeCenter[1];
                    pixelCoord[2] = uVec[2] * (i - imageCenter) + vVec[2] * (j - imageCenter)
                        + zValue; //* volume center is half of every dimensions(x,y,z,)
    ...
    void slicer2(double[] viewMatrix) {
        ...
        // perpendicular to the view vector viewVec
        double[] viewVec = new double[3];
        double[] uVec = new double[3]; // q0
        double[] vVec = new double[3]; // q1
        VectorMath.setVector(viewVec, viewMatrix[2], viewMatrix[6], viewMatrix[10]);
        VectorMath.setVector(uVec, viewMatrix[0], viewMatrix[4], viewMatrix[8]);
        VectorMath.setVector(vVec, viewMatrix[1], viewMatrix[5], viewMatrix[9]);

        /** Build all the samples (qt)
         * See R parametrization
         */
        List<double []> qts = new ArrayList<double []>();
        // TODO: Compute qts
        short t = 1;
        ...
        for (int j = 0; j < image.getHeight(); j++) {
            for (int i = 0; i < image.getWidth(); i++) {
                ...
                /* Compute all the voxels for our vectors in qt */
                for(double [] qi : qts){
                    /* Compute the st */
                    // Get the maximum
                    //
                ...
                /* Get the coordinate of the max value */
                pixelCoord[0] = uVec[0] * (i - imageCenter) + vVec[0] * (j - imageCenter)
                        + volumeCenter[0];
                pixelCoord[1] = uVec[1] * (i - imageCenter) + vVec[1] * (j - imageCenter)
                        + volumeCenter[1];
